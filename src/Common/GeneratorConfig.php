...
namespace Germey\Generator\Common;
...
class GeneratorConfig
{
    ...
    public function loadNamespaces(CommandData &$commandData)
    {
        ...
        $this->nsRepository = config('generator.namespace.repository', 'App\Repositories').$prefix;
        $this->nsModel = config('generator.namespace.model', 'App\Models').$prefix;
        $this->nsDataTables = config('generator.namespace.datatables', 'App\DataTables').$prefix;
        ...
        $this->nsApiController = config(
            'generator.namespace.api_controller',
            'App\Http\Controllers\API'
        ).$prefix;
        $this->nsApiRequest = config('generator.namespace.api_request', 'App\Http\Requests\API').$prefix;

        $this->nsRequest = config('generator.namespace.request', 'App\Http\Requests').$prefix;
        $this->nsRequestBase = config('generator.namespace.request', 'App\Http\Requests');
    ...
    public function loadPaths()
    {
        ...
        $this->pathRepository = config(
            'generator.path.repository',
            app_path('Repositories/')
        ).$prefix;

        $this->pathModel = config('generator.path.model', app_path('Models/')).$prefix;

        $this->pathDataTables = config('generator.path.datatables', app_path('DataTables/')).$prefix;

        $this->pathApiController = config(
            'generator.path.api_controller',
            app_path('Http/Controllers/API/')
        ).$prefix;

        $this->pathApiRequest = config(
            'generator.path.api_request',
            app_path('Http/Requests/API/')
        ).$prefix;

        $this->pathApiRoutes = config('generator.path.api_routes', app_path('Http/api_routes.php'));

        $this->pathApiTests = config('generator.path.api_test', base_path('tests/'));

        $this->pathApiTestTraits = config('generator.path.test_trait', base_path('tests/traits/'));

        $this->pathController = config(
            'generator.path.controller',
            app_path('Http/Controllers/')
        ).$prefix;

        $this->pathRequest = config('generator.path.request', app_path('Http/Requests/')).$prefix;

        $this->pathRoutes = config('generator.path.routes', app_path('Http/routes.php'));
    ...
    {
        $this->addOns['swagger'] = config('generator.add_on.swagger', false);
        $this->addOns['tests'] = config('generator.add_on.tests', false);
        $this->addOns['datatables'] = config('generator.add_on.datatables', false);
        $this->addOns['menu.enabled'] = config('generator.add_on.menu.enabled', false);
        $this->addOns['menu.menu_file'] = config('generator.add_on.menu.menu_file', 'layouts.menu');
    }
}

...
use Germey\Generator\Utils\TableFieldsGenerator;
...
class ModelGenerator extends BaseGenerator
{
     ...
     *
     * @param \Germey\Generator\Common\CommandData $commandData
     */
    ...
    private function fillTemplate($templateData)
    {
        ...
        if ($this->commandData->getOption('primary')) {
            $primary = generator_tab()."protected \$primaryKey = '".$this->commandData->getOption('primary')."';\n";
        } else {
            $primary = '';
        }
        ...
        $templateData = str_replace('$FIELDS$', implode(','.generator_nl_tab(1, 2), $fillables), $templateData);

        $templateData = str_replace('$RULES$', implode(','.generator_nl_tab(1, 2), $this->generateRules()), $templateData);

        $templateData = str_replace('$CAST$', implode(','.generator_nl_tab(1, 2), $this->generateCasts()), $templateData);

        $templateData = str_replace(
            '$RELATIONS$',
            fill_template($this->commandData->dynamicVars, implode(PHP_EOL.generator_nl_tab(1, 1), $this->generateRelations())),
            $templateData
        );
    ...
    private function fillSoftDeletes($templateData)
    {
        if (!$this->commandData->getOption('softDelete')) {
            ...
            $templateData = str_replace('$SOFT_DELETE$', generator_tab()."use SoftDeletes;\n", $templateData);
            $deletedAtTimestamp = config('generator.timestamps.deleted_at', 'deleted_at');
            $templateData = str_replace(
                '$SOFT_DELETE_DATES$', generator_nl_tab()."protected \$dates = ['".$deletedAtTimestamp."'];\n",
                $templateData
            );
        }
    ...
    public function generateSwagger($templateData)
    {
        ...
        $template = get_template('swagger.model.model', 'generator');
        ...
        $propertyTemplate = get_template('swagger.model.property', 'generator');
    ...
    private function fillTimestamps($templateData)
    {
        ...
        if ($this->commandData->getOption('fromTable')) {
            if (empty($timestamps)) {
                $replace = generator_nl_tab()."public \$timestamps = false;\n";
            } else {
                list($created_at, $updated_at) = collect($timestamps)->map(function ($field) {
                    return !empty($field) ? "'$field'" : 'null';
                });

                $replace .= generator_nl_tab()."const CREATED_AT = $created_at;";
                $replace .= generator_nl_tab()."const UPDATED_AT = $updated_at;\n";
            }
        }
